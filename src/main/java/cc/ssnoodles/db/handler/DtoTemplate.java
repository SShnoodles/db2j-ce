 ...
 * @author ssnoodles
 * @version 1.0
 * Create at 2020/2/2 21:00
 */
public class DtoTemplate implements Template {
    @Override
    public String render(Config config, Table table) {
        return VelocityUtil.generate(TemplateType.DTO, getContent(config, table));
    }
    ...
    public String className(String name, String rename) {
        return StringUtil.isEmpty(rename) ? StringUtil.underlineToHumpTopUpperCase(name) + "Dto" : StringUtil.underlineToHumpTopUpperCase(rename);
    }
}
